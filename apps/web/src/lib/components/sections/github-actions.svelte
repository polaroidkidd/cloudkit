<section class="mb-20">
	...
	<p class="text-xl mb-4">
		I've configured the actions using a <code>CI</code>
		and a
		<code>PREVIEW</code>
		environment. The Variables keys for both are the same (see
		<code>.env.example</code>
		). The
		<code>CI</code>
		environment values should use the same values as in the
		<code>.env.example</code>
